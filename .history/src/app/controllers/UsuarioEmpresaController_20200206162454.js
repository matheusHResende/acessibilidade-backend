...
class UsuarioEmpresaController {
  async store(req, res) {
    ...
    if (usuarioExists) {
      return res.status(404).json({ error: "Usuario ja existe." });
    } else if (emailExists) {
      return res.status(404).json({ error: "Email ja esta em uso." });
    } else if (empresaExists) {
      return res.status(404).json({ error: "cnpj ja esta em uso." });
    }
